...
- **★セキュリティ強化: 管理者操作をPOST専用に変更**
  - `doPost(e)` 追加 — 管理者APIはPOSTリクエストのみ受付
  - `doGet(e)` から管理者アクション削除 — URLにadminKeyが載らなくなった
  - adminKeyはリクエストbody（JSON）に含めて送信
- **★お知らせ画像添付機能追加**
  - 管理者がお知らせに最大5枚の画像を添付可能
  - 画像はGoogle Drive「BTM_お知らせ画像」フォルダに自動保存
  - お知らせ削除時にDrive画像も自動削除
  - お知らせシートに「画像」列を追加（5列目）
  - ★再デプロイ後、お知らせシートのE1に手動で「画像」ヘッダーを追加するか、setupSheets()を再実行
