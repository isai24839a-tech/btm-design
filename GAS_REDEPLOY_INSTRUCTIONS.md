...
## 前回デプロイからの変更点（2026-03-31更新）
- **★管理者パスワード変更**: `btmadmin2026` → `potofu7386`
- **★定期休み機能追加**:
  - 新シート「定期休み」（曜日/スタジオ/category）
  - GETエンドポイント: `regularholidays`
  - POSTエンドポイント: `addregularholiday`, `deleteregularholiday`
- **★お知らせピン留め機能追加**:
  - お知らせシートに「ピン留め」列追加（7列目）
  - POSTエンドポイント: `togglepinannouncement`
  - ピン留めお知らせが先頭にソートされる
...
- セキュリティ強化: 管理者操作をPOST専用に変更
- お知らせ画像添付機能
- ★再デプロイ後、`setupSheets()` を実行して新シート・新列ヘッダーを追加すること
